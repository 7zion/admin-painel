...
export function ProdutosCategories({ serverCategories = [] }: { serverCategories?: ProductCategory[] }) {
  const [categories, setCategories] = useState<ProductCategory[]>(serverCategories);
  const [loading, setLoading] = useState(serverCategories.length === 0);
  ...
  return (
    <section className="pt-8 pb-16 md:pb-24 bg-[#F9F8F6] overflow-hidden">
      <div className="max-w-7xl 2xl:max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        
        {loading ? (
          <div className="flex justify-center py-12">
            <div className="text-sm text-brand-text/50 animate-pulse">Carregando categorias...</div>
          </div>
        ) : categories.length > 0 ? (
          <div 
